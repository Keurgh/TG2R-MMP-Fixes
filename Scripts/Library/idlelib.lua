...
function Graveyard()
	...
	if GetSettlement("", "City") then
		...
		if BuildingGetOwner("Destination","Sitzer") then
			local Rang = (SimGetRank("")*5)+5
			CreditMoney("Destination",Rand(Rang),"tip") -- Fajeth Mod, default Rand(5)
		end
		Sleep(6)
	end
end
	...
	if GetSettlement("", "City") then
		if CityGetRandomBuilding("City", -1, 3, -1, -1, FILTER_IGNORE, "Destination") then
			...
			if not GetInsideBuilding("", "Inside") or GetID("Inside")~=GetID("HomeBuilding") then
				...
				local Carry = 0
				if GetItemCount("Destination","Wheat",INVENTORY_STD)>0 then
					--if Transfer(nil,"",INVENTORY_STD,"Destination",INVENTORY_STD,"Wheat",1) then
						Carry = 1
					--end
				end
				if Carry == 1 then
		...
		if CityGetRandomBuilding("City", 5,14,Market,-1, FILTER_IGNORE, "Destination") then
			...
			if not GetInsideBuilding("", "Inside") or GetID("Inside")~=GetID("HomeBuilding") then
				...
				if Choice == 0 then
					...
					elseif zItem == 3 then
						Itemchoice = "FriedHerring"
					elseif zItem == 4 then
						Itemchoice = "Knochenarmreif"
					end
				...
				elseif Choice == 2 then
					Ware = "Handheld_Device/ANIM_Tailorbasket.nif"
						if zItem == 0 then
						Itemchoice = "LeatherGloves"
					elseif zItem == 1 then
						Itemchoice = "MoneyBag"
					elseif zItem == 2 then
						Itemchoice = "Blanket"
					elseif zItem == 3 then
						Itemchoice = "SmokedSalmon"
					elseif zItem == 4 then
						Itemchoice = "Schnitzi"
					end
				...
				CityGetRandomBuilding("City", 5,14,Market,-1, FILTER_IGNORE, "Market")
...
function CheckInsideStore()
  ...
  local store = Rand(10) --5 Fajeth Mod
	GetSettlement("", "City")
	local Wares = {}
	-- backery
	if store == 0 then
		Wares = {"Cookie","Wheatbread"}
		...
		if GetItemCount("backerei", Choice, INVENTORY_STD)>0 then
		    if Rand(2) == 0 then
				...
				RemoveItems("backerei",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("backerei",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
		else
			...
			if BuildingGetOwner("backerei","Cheffi") then
				...
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_HPFZ_IDLELIB_GETFOOD_SPRUCH_+2",prodNam)
			end
		end
		SatisfyNeed("", 1, 0.15)
		-- tailor
	elseif store == 1 then
		Wares = {"FarmersClothes","Blanket"}
		local Choice = Wares[(Rand(2)+1)]
		...
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("schneiderei", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			  MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
				...
				RemoveItems("schneiderei",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("schneiderei",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
			...
			if BuildingGetOwner("schneiderei","Cheffi") then
				...
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+3",prodNam)
			end
		end
		SatisfyNeed("", 7, 0.15)
		-- joinery
	elseif store == 2 then
		Wares = {"Torch","OakwoodRing"}
		local Choice = Wares[(Rand(2)+1)]
		...
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("tischler", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			    MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
			else
				...
				RemoveItems("tischler",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("tischler",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
	...
	elseif store == 3 then
		...
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("schmied", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			    MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
			else
				...
				RemoveItems("schmied",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("schmied",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
	...
	elseif store == 4 then
		...
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("smetz", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			    MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
			else
				...
				RemoveItems("smetz",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("smetz",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
	...
	elseif store == 5 then
		Wares = {"HerbTea","Dye"}
		local Choice = Wares[(Rand(2)+1)]
		if not CityGetRandomBuilding("City",3,16,-1,-1,FILTER_HAS_DYNASTY,"alchimist") then
		    return
		end
		if not f_MoveTo("","alchimist",GL_MOVESPEED_RUN,100) then
		    return
		end
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("alchimist", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			    MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
			else
				...
				if BuildingGetOwner("alchimist","Cheffi") then
				RemoveItems("alchimist",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("alchimist",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
	...
	elseif store == 6 then
		Wares = {"Housel","Poem"}
		local Choice = Wares[(Rand(2)+1)]
		if not CityGetRandomBuilding("City",3,19,-1,-1,FILTER_HAS_DYNASTY,"church_ev") then
		    return
		end
		if not f_MoveTo("","church_ev",GL_MOVESPEED_RUN,100) then
		    return
		end
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("church_ev", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			    MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
			else
				...
				if BuildingGetOwner("church_ev","Cheffi") then
				RemoveItems("church_ev",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("church_ev",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
		...
		Wares = {"Housel","Poem"}
		local Choice = Wares[(Rand(2)+1)]
		if not CityGetRandomBuilding("City",3,20,-1,-1,FILTER_HAS_DYNASTY,"church_ca") then
		    return
		end
		if not f_MoveTo("","church_ca",GL_MOVESPEED_RUN,100) then
		    return
		end
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("church_ca", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			    MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
			else
				...
				if BuildingGetOwner("church_ca","Cheffi") then
				RemoveItems("church_ca",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("church_ca",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
		...
		Wares = {"Soap","Salve"}
		local Choice = Wares[(Rand(2)+1)]
		if not CityGetRandomBuilding("City",3,37,-1,-1,FILTER_HAS_DYNASTY,"hospital") then
		    return
		end
		if not f_MoveTo("","hospital",GL_MOVESPEED_RUN,100) then
		    return
		end
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("hospital", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			    MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
			else
				...
				if BuildingGetOwner("hospital","Cheffi") then
				RemoveItems("hospital",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("hospital",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
		...
		Wares = {"Schadelkerze","Knochenarmreif"}
		local Choice = Wares[(Rand(2)+1)]
		if not CityGetRandomBuilding("City",3,98,-1,-1,FILTER_HAS_DYNASTY,"necro") then
		    return
		end
		if not f_MoveTo("","necro",GL_MOVESPEED_RUN,100) then
		    return
		end
		local prodNam = ItemGetLabel(Choice,true)
		if GetItemCount("necro", Choice, INVENTORY_STD)>0 then
			PlayAnimationNoWait("","manipulate_middle_twohand")
			if Rand(2) == 0 then
			    MsgSay("","@L_HPFZ_IDLELIB_GETGOOD_SPRUCH_+0",prodNam)
			else
				...
				if BuildingGetOwner("necro","Cheffi") then
				RemoveItems("necro",Choice,1,INVENTORY_STD)
				chr_ModifyFavor("","Cheffi",5)
				local bonus = (GetSkillValue("Cheffi", BARGAINING)/10)*ItemGetBasePrice(Choice)+ItemGetBasePrice(Choice)
				Sleep(0.5)
				CreditMoney("necro",bonus,"tip")
				feedback_MessageCharacter("Cheffi",
				"@L_MEASURES_BUYSTUFF_HEAD",
				"@L_MEASURES_BUYSTUFF_BODY",prodNam, bonus)
				end
...
function GoToTavern()
	...
	-- Fajeth_Mod TavernMod
		local simrang = SimGetRank("")
		local beer = 0

	if HasProperty("Destination","ServiceActive") then
		if simrang == 0 or simrang == 1 then
	    	beer = Rand(5)+5
		elseif simrang == 2 then
		beer = Rand(10)+10
		elseif simrang == 3 then
		beer = Rand(15)+15
		elseif simrang == 4 then
		beer = Rand(20)+20
		elseif simrang == 5 then
		beer = Rand(25)+25
		end				
	else
		if simrang == 0 or simrang == 1 then
	...
	-- Mod end
		...
		while verweile > 0 do
			...
			if SimGetNeed("", 8) >  SimGetNeed("", 1) then
				--Mod by Fajeth_Same money as divehouse
				CreditMoney("Destination",beer,"Misc")
				-- Mod end
			...
			if SimGetNeed("", 8) > 0.3 or  SimGetNeed("", 1) > 0.3 then
				...
				if GetItemCount("Destination", Choice, INVENTORY_STD)>0 then
					Transfer(nil, nil, INVENTORY_STD, "Destination", INVENTORY_STD, Choice, NumItems)
...
function VisitDoc(HospitalID)
	...
	if not AliasExists("Destination") then	
		...
		for i=0,NumHospitals-1 do
			...
			local MinLevel = 1
			-- Fajeth Mod Hospital has all medicine at lvl 1
	...
	if ((GetImpactValue("","Sickness")>0) or (GetHP("") < GetMaxHP(""))) then
		...
		-- Fajeht Mod: Be more patient
		local Waittime = GetGametime() + 6 -- +3
...
function GoToDivehouse()
	...
	if HasProperty("Destination","ServiceActive") then
		...
		grundBetrag = Rand(10)+10
		elseif simstand == 3 then
		grundBetrag = Rand(15)+15
		elseif simstand == 4 then
		grundBetrag = Rand(20)+20
		elseif simstand == 5 then
		grundBetrag = Rand(25)+25
		end				
	else
		if simstand == 0 or simstand == 1 then
		grundBetrag = 5
...
--			if SimGetNeed("", 8) > 0.2 then
				...
				if GetItemCount("Destination", Choice, INVENTORY_STD)>0 then
					if Choice == "PiratenGrog" or Choice == "Schadelbrand" then
						RemoveItems("Destination",Choice,NumItems,INVENTORY_STD)
						if Choice == "PiratenGrog" then
						-- Fajeth Mod: More Gold for Divehouse
							CreditMoney("Destination",50,"Offering")
						else
							CreditMoney("Destination",100,"Offering")
						end
					else
						Transfer(nil, nil, INVENTORY_STD, "Destination", INVENTORY_STD, Choice, NumItems)
					end
