function Init()
	...
	local filter
	if ResourceGetItemId("Destination") == 4 then
		filter ="__F( (Object.GetObjectsByRadius(Sim)==700)AND(Object.GetProfession()==58))" -- Pig
	elseif ResourceGetItemId("Destination") == 7 then
	filter ="__F( (Object.GetObjectsByRadius(Sim)==700)AND(Object.GetProfession()==55))" --  Sheep
	elseif ResourceGetItemId("Destination") == 9 then
	filter ="__F( (Object.GetObjectsByRadius(Sim)==700)AND(Object.GetProfession()==57))" -- Cattle	
	end
	if filter then
...
function AIInit()
    ...
    if BuildingHasUpgrade(betrieb,68) == true then -- Sheep
	    weights[1] = 0
	end
	if BuildingHasUpgrade(betrieb,64) == true then -- Pig
	    weights[2] = 0
	end
	if BuildingHasUpgrade(betrieb,65) == true then -- Cattle
	    weights[3] = 0
	end
	...
	for v=0,slots-1 do
		...
		if id == 7 and weights[1] == 0 then
		    weights[1] = menge
		end
		if id == 4 and weights[2] == 0 then
		    weights[2] = menge
		end
		if id == 9 and weights[3] == 0 then
		    weights[3] = menge
		end
	...
	elseif (weights[2] < weights[1]) and (weights[2] < weights[3]) then
	    round = 2
	elseif (weights[3] < weights[1]) and (weights[3] < weights[2]) then
	    round = 3
	else
	round = Rand(3)+1
	end
